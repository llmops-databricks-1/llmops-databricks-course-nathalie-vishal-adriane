...
# dependencies = [
#   "/Workspace/Users/vishal.krajput108@gmail.com/llmops-databricks-course-nathalie-vishal-adriane",
# ]
# ///
# MAGIC %md
...
# MAGIC
# MAGIC ### RAG Pipeline Overview (Offline — Data Ingestion & Indexing)
# MAGIC
# MAGIC This is the **setup phase** — preparing your knowledge base before any user query:
# MAGIC
# MAGIC ```
# MAGIC Raw Documents (PDFs, HTML, Docs, etc.)
# MAGIC     ↓
# MAGIC Document Parsing (extract text, tables, images)
# MAGIC     ↓
# MAGIC Chunking (split into meaningful segments)
# MAGIC     ↓
# MAGIC Embedding (convert chunks to vectors)
# MAGIC     ↓
# MAGIC Vector Search Index (store embeddings + metadata)
# MAGIC     ↓
# MAGIC Vector Search Endpoint (serve the index for queries)
# MAGIC ```
# MAGIC
# MAGIC This pipeline runs **periodically** (or on data change) to keep the knowledge base up to date.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### RAG Query Flow (Online — User Inference)
# MAGIC
# MAGIC This is the **runtime phase** — what happens each time a user asks a question:
# MAGIC
# MAGIC ```
# MAGIC User Query
# MAGIC     ↓
# MAGIC Query Rewriting (generate alternate phrasings via LLM → improves recall)
# MAGIC     ↓
# MAGIC Query Embedding (embed original + rewritten queries)
# MAGIC     ↓
# MAGIC Vector Search (retrieve relevant chunks for ALL query variations)
# MAGIC     ↓
# MAGIC Deduplicate & Merge results
# MAGIC     ↓
# MAGIC Context Assembly (order, filter, compress)
# MAGIC     ↓
# MAGIC LLM Prompt (query + retrieved context)
# MAGIC     ↓
# MAGIC Generated Response
# MAGIC ```
# MAGIC
# MAGIC This flow executes **per request** and depends on the pipeline above being set up first.
# MAGIC
